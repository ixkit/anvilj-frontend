<template>
  <div ref="wrapRef" :class="getWrapperClass">
    ...
    <a-form-item-rest>      
      ...
      <!-- @@ card style begin -->
      <BasicCardGrid ref="cardGridRef"  v-bind="getBindValues" :prefixClsCard="prefixClsCard" >  
        <template #itemCardRaw="{ item }"  v-if="$slots.itemCard"> 
          <slot name="itemCard"  v-bind:item="item"></slot> 
        </template> 
      </BasicCardGrid>
      <!-- @@ card style end -->        
    </a-form-item-rest>
...
<script lang="ts">
  //@@ card style 
  import BasicCardGrid from './card/BasicCardGrid.vue';
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, emit, slots, expose }) {
      ...
      //@@ card style begin 
      const prefixClsCard = getProps.value.prefixClsCard;
      const { getViewStyle, setViewStyle } = useViewStyle(getProps);
      //@@ card style end
      ...
      const handlers: InnerHandlers = {
        ...
        //@@ card style
        onSwitchViewStyle: (style: string) => {
          console.log('inner handler onSwitchViewStyle',style);
          switchViewStyle();
          emit('on-view-style-change', style);
          unref(getProps).onSwitchViewStyle?.(style);
        }       
      };
      ...
      const getBindValues = computed(() => {
        ...
        let propsData: Recordable = {
          ...
          //switch view style: list | card 
          viewStyle: unref(getViewStyle),
        ...
        //@@ card style, fix set view style if use cardtable
        propsData.tableSetting.showViewStyle = true;
      ...
      //@@ card style functions begin  
      function getCardGridContainer(){
        const elt = cardGridRef.value.gridContainer;
        return elt;
      } 
      
      function _setTableHeaderAndBodyVisiable(showCard){
        
        const header = wrapRef.value.querySelectorAll('.ant-table-header');
        console.log('find header', header);
        const table_display_style = showCard ?  "none" : "block";
        let handleCount = 0;
        header.forEach((elem) => {
                    elem.style.display = table_display_style;
                    handleCount = handleCount + 1;
            }); 
         
        const tableBody = wrapRef.value.querySelectorAll('.ant-table-body');
        console.log('find tableBody', tableBody);
      ...
      function setTableVisiable(){
        console.log('manual setTableVisiable');
        let viewStyle = getViewStyleValue();
        let showCard = viewStyle == 'card';
        const tableContent = wrapRef.value.querySelectorAll('.ant-table-content');
        const table_display_style = showCard ?  "none" : "block";
        let handleCount = 0;
        tableContent.forEach((elem) => {
                    elem.style.display = table_display_style;
                    handleCount = handleCount + 1;
                    console.log('tableContent??',handleCount);  
            }); 
        
        const handleTableBody = _setTableHeaderAndBodyVisiable(showCard);
        return handleCount + handleTableBody;
      }

      function renderViewStyle(val){
      
        console.log('try renderViewStyle',val);  
       
        let showCard = val == 'card'; 
        const cardElt = getCardGridContainer();
        //console.log('cardElt?',cardElt);
        cardElt.style.display =  showCard ? "block": "none";
        //force render card grid    
        const card = unref(cardGridRef.value); 
        card.cardVisible = showCard;

        const handleCount = setTableVisiable();
        console.log('handleCount?',handleCount);
        if (handleCount <=1 )
        {
          console.log('dealy setTableVisiable');
          setTimeout(()=>{
            console.log('try exec setTableVisiable');
            setTableVisiable();
          },1000*1)
        } 
      }
      function putCardInTableContainer(){
        const container = wrapRef.value.querySelectorAll('.ant-table-container')[0]; 
        const elt = getCardGridContainer();
        elt.removeAttribute('title');
        console.log('try putCardInTableContainer', container);
        container.appendChild(elt); 
      }
      const setViewStyleValue = (style: string) => {
        setViewStyle(style);
      };
      const getViewStyleValue = () => {
        const values = unref(getBindValues);
        let viewStyle = values.viewStyle;
        return viewStyle;
      };
      const switchViewStyle = function(){ 
        let viewStyle = getViewStyleValue();
        console.log('try switchViewStyle,current:',viewStyle);
        viewStyle =  viewStyle == 'card' ? 'list' : 'card';
      
        setViewStyleValue(viewStyle);
        renderViewStyle( viewStyle);

      };
      //main entry
      onMounted(() => {
        console.log('onMounted',getBindValues);
        putCardInTableContainer();
        const viewStyle = getViewStyleValue();
        renderViewStyle( viewStyle);
      })
      //@@ card style function end
